...
This tutorial explores how to handle **spatial lines** in {{< fa brands r-project >}} with `terra` package:
...
Otters live along rivers, but how far from the rivers were they observed? 
...
Lines are made of multiple points. It is possible to create lines directly from coordinates but, in practice, it often comes from an existing spatial dataset. In our example, we will load rivers for the area of interest from [IGN data BD CARTO](https://geoservices.ign.fr/bdcarto).

*Note that this dataset has rough resolution ([BD TOPO](https://geoservices.ign.fr/bdtopo) would be recommended for real analysis), but it's perfect for our illustration and learning purpose.* 
...
In `sf`, there are [two functions](https://search.r-project.org/CRAN/refmans/sf/html/st_read.html) to read vector data `sf::st_read()` and `sf::read_sf()`. The main difference lies in the format of the attribute table: it is stored as `data.frame` with `st_read`, and as a `tibble` with `read_sf()`. 
...
1. How many different stretches of river was loaded?
2. What is the coordinate reference system (CRS) of the loaded river data?
3. Can you get the name of the all the river stretches?
:::

<details>
<summary>Click to see the answer</summary>

1. There are ```r nrow(river)``` stretches (=lines) in the dataset. You can access it with `dim(river)`, `nrow(river)` or just by typing `river` in the console.  
...
When calculating distances and length, **be careful with projection systems**. Some are not suited to calculate distance, prefer equidistance projections or use local projection systems. Recent implementation of `terra` and `sf` calculates [spherical distances](https://en.wikipedia.org/wiki/Great-circle_distance) when using geographic coordinates (in longitude and latitude) which is the most accurate and recommended to account for Earth's curvature.
...
In `terra`, use the argument `add=TRUE` or the function [`terra::points()`](https://search.r-project.org/CRAN/refmans/terra/html/lines.html) to add points to an existing map.
```{r mapterra_line}
plot(
  river,
  y = "length_km",
...
To calculate distance among objects, we use the function [`terra::distance()`](https://search.r-project.org/CRAN/refmans/terra/html/distance.html).  

Before comparing two spatial objects, it is recommended to plot them (as in the [previous figure](https://frbcesab.github.io/spatial-r/chapters/12_lines_files/figure-html/mapterra_line-1.png)) and make sure their projection systems are the same and the extents match. *Do not use the package `mapview` because it will automatically project the datasets.*
...
In `sf`, the same procedure as in `terra` can be followed with `sf::st_distance()`. However it is much faster to use a two-step process: (1) find the closest river with [`sf::st_nearest_feature()`](https://search.r-project.org/CRAN/refmans/sf/html/st_nearest_feature.html) and (2) calculate the distance between points and their closest river with [`sf::st_distance()`](https://search.r-project.org/CRAN/refmans/sf/html/geos_measures.html).  
...
:::{.callout-note title="Your turn"}
1. Which are the rivers with most sights of otters?  
2. Were otters observed close to small or large rivers? 
3. Make a map with the distance to river and visually check the coherence of the calculation.  
4. (Reflexion) What can be the factors explaining the few outliers with large distances (>1km)?
:::
...
```{r ex3}
# add an attribute whether an otter was observed near the river in 2021
river$otter_sight <- ifelse(1:nrow(river) %in% nearest_river, "yes", "no")
# compare the distribution of river lengths
boxplot(
  river$length_km ~ river$otter_sight,
  xlab = "Otter sighting",
  ylab = "River length (km)"
)
```

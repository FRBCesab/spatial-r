...
The linear model is definitely not a good fit to our data (R2 is only 0.03). Moreover, there is strong collinearity between elevation and temperature, and the residuals are not normally distributed. 
It might help if we use generalized mixed models with `poisson` or `quasipoisson` distribution for count data. Yet, the main issue here is the high number of 'false' zeros (opportunistic observations, presence-only information).  

Additionally, it is always recommended to spatially visualize the predictions and the residuals.

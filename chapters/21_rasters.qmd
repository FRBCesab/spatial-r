...
This tutorial explore how to handle **rasters** in {{< fa brands r-project >}} with `terra` package:
...
:::{.callout-tip title="The ecologist mind"}
To avoid considering regions outside the habitats, it is recommended to mask areas that are not part of the habitats (such as the sea for otter). 
:::
...
Be aware of the projection. It is recommended to work on lat/longitude, or in an equal-area projection (which is not the case of Lambert-93). So we will to create the buffer in lat/long and project them in Lambert-93 afterward.

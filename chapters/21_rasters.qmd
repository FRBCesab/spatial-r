...
Elevation is a key geographical variable that strongly define habitats. At which elevation were the otters observed?
...
The function to read raster file is [`terra::rast()`](https://search.r-project.org/CRAN/refmans/terra/html/rast.html)
...
We can access all values of the raster with the function [`terra::values()`]((https://search.r-project.org/CRAN/refmans/terra/html/values.html)). This operation is often not needed and not recommended if you have large rasters. But it is the only way to get the distribution of all values.
...
To mask all pixels that don't fit our study area (the terrestrial environment 50km around Montpellier), we need (1) to get the borders of France from [GADM](https://gadm.org/), (2) project the border to match the elevation raster, and (3) mask the area that are not in the country polygon with the function [`terra::mask()`](https://search.r-project.org/CRAN/refmans/terra/html/mask.html).

...
We will use four packages: `terra`[@R-terra] and `sf`[@R-sf] for handling spatial data, `mapview`[@R-mapview] for interactive maps, and `here`[@R-here] for locating files locally.
...
As an toy dataset, the tutorials rely on observed occurrence of otter in 2021 around Montpellier, France. These dataset were downloaded directly from {{< fa brands r-project >}}, for more details see the [tutorial on creating a toy dataset from GBIF, IGN and CHELSA](https://frbcesab.github.io/spatial-r/chapters/31_toydata_mpl.html).

...
This tutorial explores how to handle **spatial points** in {{< fa brands r-project >}} with `terra` package:

- create a spatial object from a data.frame with [`terra::vect()`](https://search.r-project.org/CRAN/refmans/terra/html/vect.html) and export it with [`terra::writeVector()`](https://search.r-project.org/CRAN/refmans/terra/html/writeVector.html)  
- make interactive maps with [`mapview::mapview()`](https://search.r-project.org/CRAN/refmans/mapview/html/mapView.html)
:::


:::{.callout-tip title="The ecologist mind"}
From the otters observations of GBIF, we want to create a map. We will need to create a proper spatial object in {{< fa brands r-project >}}. 
:::
...
This is the most common case: you have coordinates from a file (csv or excel) and you want to get spatial information about these locations. The first step is to transform the coordinates as proper spatial object in {{< fa brands r-project >}}.
...
We have just loaded in R the observations from GBIF, but it is stored in a simple `data.frame` object, {{< fa brands r-project >}} doesn't know that it contains spatial information yet. Let's **create a spatial object from the coordinates** so that we can easily make a map and see where these observation were made.
...
## mapview
The function [`mapview::mapview()`](https://search.r-project.org/CRAN/refmans/mapview/html/mapView.html) creates easily interactive map. If you want to fine tune your map, have a look at this [tutorial](https://r-spatial.github.io/mapview/articles/mapview_01-basics.html)
...
## terra
The function [`plot()`](https://search.r-project.org/CRAN/refmans/terra/html/plot.html)
. 
```{r terra_map0}
plot(pt_terra)
```
...
The date formatting in R is powerful but hard to grasp. Make sure to have a look at the documentations of [`as.Date()`](https://search.r-project.org/R/refmans/base/html/as.Date.html) and [`strptime()`](https://search.r-project.org/R/refmans/base/html/strptime.html) if you want more details.  


```{r julian}
# transform the date as Julian day
julian <- as.Date(pt_terra$eventDate) |> format("%j") |> as.numeric()
```
...
```{r mapview_julian}
mapview(pt_terra, zcol = "julian", layer.name = "Julian date")
```

## terra

```{r terra_map_julian}
plot(pt_terra, "julian", type = "continuous", main = "Julian date")
```
...
For vectors, it is [recommended](https://frbcesab.github.io/spatial-slides/intro-GIS#/vector-vs-raster-file-format) to export them as **geopackage file** (extension `.gpkg`). Compare to traditional ESRI shapefile, the geopackage format stores the data in a single file and the column names are preserved. 


::: {.panel-tabset}

## terra
In `terra`, the function to export a spatial vector is [`terra::writeVector()`](https://search.r-project.org/CRAN/refmans/terra/html/writeVector.html).
...
## sf
In `sf`, the function to export a spatial vector is [`sf::st_write()`](https://search.r-project.org/CRAN/refmans/sf/html/st_write.html).
...
## from terra to sp
To convert a `terra SpatVector` vector into a `sf simple feature`, the function is [`sf::st_as_sf()`](https://search.r-project.org/CRAN/refmans/sf/html/st_as_sf.html).
...
## from sf to terra
To convert a `sf simple feature` into `terra SpatVector`, the function is [`terra::vect()`](https://search.r-project.org/CRAN/refmans/terra/html/vect.html).

...
We will use the package `rgbif`[@R-rgbif] which provides a good interface to access directly [GBIF](https://www.gbif.org/) data. A good way is to start with the function [`rgbif::occ_count()`](https://search.r-project.org/CRAN/refmans/rgbif/html/occ_count.html) to know how many records fits our criteria.
...
### Data download
If the dataset is small (<100.000 records), then we can use the function [`rgbif::occ_search()`](https://search.r-project.org/CRAN/refmans/rgbif/html/occ_search.html). Else we would need to use the function `rgbif::occ_download()` and register in GBIF. For the sake of downloading a small toy dataset, this is not needed.

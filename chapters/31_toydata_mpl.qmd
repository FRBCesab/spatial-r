...
For this tutorial, we will need `sf`, `mapview`, `happign`, `geodata`, `terra` and `rgbif` packages.
...
We will use the package `rgbif`[@R-rgbif] which provides a good interface to access directly [GBIF](https://www.gbif.org/) data. It is recommended to start with the function [`rgbif::occ_count()`](https://search.r-project.org/CRAN/refmans/rgbif/html/occ_count.html) to know how many records fits the search criteria.
...
### Data download
If the dataset is small (<100.000 records), then you can use the function [`rgbif::occ_search()`](https://search.r-project.org/CRAN/refmans/rgbif/html/occ_search.html). Else you need to use the function [`rgbif::occ_download()`](https://search.r-project.org/CRAN/refmans/rgbif/html/occ_download.html) and register in GBIF. For the sake of downloading a small toy dataset, this registration step is not needed.

:::{.callout-important}
For real analysis, it is recommended to use `rgbif::occ_download()` which will provide data with a DOI that can be directly cited in future publications.
:::
...
The package `happign`[@R-happign] provides a great interface to download [French spatial data from IGN](https://geoservices.ign.fr/catalogue) using protocols such as Web Map Service (WMS) for raster and Web Feature Service (WFS) for vectors. Read the [documentation](https://paul-carteron.github.io/happign/index.html) for more details. 

It is recommended to extend the buffer to make sure all points and their buffers are covered with data. We will add 0.1 degree of extra margin to the original buffer.
```{r large_box}
...
### Elevation from BD ALTI
We will use the function `get_wms_raster()`. For quick testing, it is recommended to use interactive = TRUE. In our case we want `altimetrie` data from [BD ALTI](https://geoservices.ign.fr/bdalti) at 25m resolution. For the sake of the example, we will download the dataset with a resolution of 250m to make the file smaller and fasten computation.
...
```{r load_elevation}
#| echo: false
mnt_2154 <- rast(here::here("data", "BDALTI_mpl50km.tif"))
```
...
# as shapefile
st_write(landcover, here::here("data", "BDCARTO-LULC_mpl50km.shp"))
```

```{r load_landcover}
#| echo: false
landcover <- st_read(here::here("data", "BDCARTO-LULC_mpl50km.shp"))
```

```{r map_landcover}
plot(landcover["nature"], reset = FALSE, border = FALSE)
...
# as GeoPackage
st_write(river, here::here("data", "BDCARTO-River_mpl50km.gpkg"))
```

```{r load_river}
#| echo: false
river <- st_read(here::here("data", "BDCARTO-River_mpl50km.gpkg"))
```

```{r map_river}
plot(st_geometry(river), reset = FALSE, col = "blue", axes = TRUE)
...
# 4. Get climate data from Chelsa

[Chelsa](https://www.chelsa-climate.org/) is a good source of climate data at world scale with 1km spatial resolution. 
To download multiple files on a selected region, we can use the following home-made R-script:  
...
# export
writeRaster(crop_tas, here::here("data", "CHELSA_monthly_tas_2015_2021.tif"))
```

```{r load_temperature}
#| echo: false
crop_tas <- rast(here::here("data", "CHELSA_monthly_tas_2015_2021.tif"))
```

```{r map_temperature}
plot(crop_tas, y = grep("_2021_", names(crop_tas)), fun = function() {
...
<!--
### E-Obs or ERA5 dataset
[E-obs](https://surfobs.climate.copernicus.eu/dataaccess/access_eobs.php) is a good source of daily weather for Europe, at 0.1 degree resolution (~10km).
For our example, we download files for [chunks of 15 years](https://surfobs.climate.copernicus.eu/dataaccess/access_eobs_chunks.php).

```{r}
#| eval: false
# RR: daily precipitation sum
url_eobs <- "https://knmi-ecad-assets-prd.s3.amazonaws.com/ensembles/data/Grid_0.1deg_reg_ensemble/rr_ens_mean_0.1deg_reg_2011-2024_v31.0e.nc"
# get the full dataset
prec_eobs <- terra::rast(url_eobs)
# crop to the extent of intest
crop_prec <- terra::crop(prec_eobs, large_box)
# export
writeRaster(crop_prec, here::here("data", "EOBS_daily_rr_2011_2024.nc"))
```
...
[GADM](https://gadm.org/), the Database of Global Administrative Areas, is a high-resolution database of country administrative areas. It can be easily accessed through the package `geodata`[@R-geodata].
...
```{r load_mask}
#| echo: false
france_border <- vect(here::here("data", "gadm41_FRA_0_pk.rds"))
```

...
With `terra`, when large geotiff files are stored as [Cloud optimized Geotiff](https://cogeo.org/), you don't have to download them all.
The trick is to add `/vsicurl/` before the url and load them as a nornmal raster. For instance, with CHELSA dataset. Then you can crop or extract values without downloading the whole dataset.
